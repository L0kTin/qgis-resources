        ...
        for current, feature in enumerate(features):
            ...
            if verticesNum == 5:
                if feature['neighbor'] != 0:
                    ...
                    southPointArray = []
                    southPointArray.append(pointarray[southPointindexmin])
                    southPointArray.append(pointarray[southPointindexmax])

                    # 找最北的两个点
                    northPointArray = []
                    pointArrayLen = len(pointarray)-1
                    for index in range(pointArrayLen):
                        if index != southPointindexmin and index != southPointindexmax:
                            northPointArray.append(pointarray[index])

                    # 找到最南和最北两个点的index，如果和邻居南北有重合，就生成在左右
                    # 判断是否不是南北排列的
                    ...
                    poinlen = len(southPointArray)
                    for index in range(poinlen):
                        for index2 in range(neipointLen) :
                            point1 = QgsPointXY(southPointArray[index].x(),southPointArray[index].y())
                            point2 = QgsPointXY(neighborarray[index2].x(),neighborarray[index2].y())
                    ...
                    if len(neighborpointindex) != 2:
                        neighborpoint = []
                        neighborpointindex = []
                        poinlen = len(northPointArray)
                        for index in range(poinlen):
                            for index2 in range(neipointLen) :
                                point1 = QgsPointXY(northPointArray[index].x(),northPointArray[index].y())
                                point2 = QgsPointXY(neighborarray[index2].x(),neighborarray[index2].y())
                                m = distance.measureLine(point1, point2)
                                if m  < 10:
                                    neighborpoint.append(pointarray[index])
